...
const char* ssid = "Infinix NOTE 30ss";
...
const char* topic_moisture = "/sensor/data/moisture";
...
void loop() {
  ...
  if (now - lastMsg > 2000) { //perintah publish data
    ...
    int valueMoist = analogRead(AOUT_PIN);
    ...
    String moist_string = String(moist, 3);
    client.publish(topic_moisture, moist_string.c_str());
